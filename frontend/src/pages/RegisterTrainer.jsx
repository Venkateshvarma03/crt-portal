...
function RegisterTrainer() {
  const [form, setForm] = useState({
    ...
    password: '',
    specialization: '',
    batches: '', // comma-separated input, we'll split into an array
  })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await api.post('/auth/trainer/register', payload)
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#f8fafc] py-10">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-sm border border-[#f1f5f9] w-full max-w-md">
        <h1 className="text-xl font-bold text-gray-900 mb-1">Trainer Registration</h1>
        <p className="text-sm text-gray-500 mb-6">Create your Trainer account</p>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="text-xs font-medium text-gray-600">Batches (comma separated)</label>
            <input
              type="text"
              name="batches"
              value={form.batches}
              onChange={handleChange}
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm mt-1"
              placeholder="e.g. CSE A - 2026, CSE B - 2026"
            />
          </div>
        </div>
